...
class RecoveryBluetooth : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (getFileStreamPath(".auto" + this@RecoveryBluetooth.javaClass.simpleName.replace("Recovery", "")).exists()
            ...
            val packageNames = fileIO.readFileLinesAsArray(".auto" + this@RecoveryBluetooth.javaClass.simpleName.replace("Recovery", ""))
            if (!packageNames.isNullOrEmpty()) {
                for (aPackageName in packageNames) {
                    floatingServices.runUnlimitedShortcutsForBluetooth(aPackageName)
                }
            }
        }
        ...
        if (getFileStreamPath(".auto" + this@RecoveryBluetooth.javaClass.simpleName.replace("Recovery", "") + "Category").exists()
            ...
            val folderNames = fileIO.readFileLinesAsArray(".auto" + this@RecoveryBluetooth.javaClass.simpleName.replace("Recovery", "") + "Category")
            if (!folderNames.isNullOrEmpty()) {
                for (folderName in folderNames) {

                    fileIO.readFileLinesAsArray(folderName)?.let {

                        floatingServices.runUnlimitedFoldersForBluetooth(folderName, it)
                    }
                }
            }
        }
